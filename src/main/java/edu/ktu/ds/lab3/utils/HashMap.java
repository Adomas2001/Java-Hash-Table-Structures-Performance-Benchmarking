...
public class HashMap<K, V> implements EvaluableMap<K, V> {
    ...
    public V remove(K key) {
        ...
        Node<K,V> nodeBeDeleted = new Node<>();
        nodeBeDeleted.key=key;
        nodeBeDeleted.value = get(key);

        int index = HashManager.hash(key.hashCode(), table.length, ht);
        Node<K,V> iterator=table[index];

        if(iterator.key==nodeBeDeleted.key)
        {
    ...
    public boolean checklinkedlistnull(Node<K,V> node)
    {
        boolean tf=true;
